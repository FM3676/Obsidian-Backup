...
draft: false
summary: "This article introuduces React rendering process"
layout: PostSimple
...
![React-rendering-process-1](Images/React/React-rendering-process/React-rendering-process-1.png)
...
![React-rendering-process-2](Images/React/React-rendering-process/React-rendering-process-2.png)
...
![React-rendering-process-3](Images/React/React-rendering-process/React-rendering-process-3.png)

打开控制台，可以看到他会打印出当前帧绘制完毕后所剩余的时间，但值得注意的是，有的是 16 以内，但也有大于 16 例如 49、50 的，为什么会这样呢？
...
执行结果如下，可以发现，基本上每一帧的剩余时间都不会大于 16.66 了。
![React-rendering-process-4](Images/React/React-rendering-process/React-rendering-process-4.png)

### Simulate 'Scheduler'

我们可以通过`requestIdleCallback`去模拟 `Scheduler` 的工作流程
...
![React-rendering-process-5](Images/React/React-rendering-process/React-rendering-process-5.png)

可以看到，每一个任务就在剩余的空闲时间中穿插执行，这样就避免了阻塞。
...
function performUnitOfWork(fiberNode) {
  // 省略 beginWork
  if (fiberNode.child) performUnitOfWork(fiberNode.child);

  // 省略 CompleteWork
  if (fiberNode.sibling) performUnitOfWork(fiberNode.sibling);
}
```

最后来看一张图：

![React-rendering-process-6](Images/React/React-rendering-process/React-rendering-process-6.png)
...
![React-rendering-process-7](Images/React/React-rendering-process/React-rendering-process-7.png)

---
title: 'React Scheduler Task Scheduling'
date: 2023-12-28T23:08:14Z
lastmod: '2023-12-28'
tags: ['React', 'Behind The Scene', 'Tutorial']
draft: false
summary: 'An overview of React Scheduler task scheduling using priority queues and timing for efficient task execution.'
layout: PostSimple
---
...
function unstable_scheduleCallback(priorityLevel, callback, options) {
  ...
  // 接下来就计算出过期时间
  // 计算出来的时间有些比当前时间要早(IMMEDIATE_PRIORITY_TIMEOUT )，绝大部分比当前的时间要晚一些
  // 下文有图解搭配
  ...
  /* 
  GPT：在 React 源代码中，enableProfiling 是一个标志，用于在开发环境中启用额外的性能追踪和分析特性。这些特性可以帮助开发者了解应用程序的性能瓶颈，特别是在组件渲染和更新时的性能表现。
  */
  if (enableProfiling) {
...
![React-Scheduler-Task-Scheduling-1](/Images/React/React-Scheduler-Task-Scheduling/React-Scheduler-Task-Scheduling-1.png)
...
- 当然，不是说一直从任务队列里面取任务出来执行就完事儿，每次取出一个任务后，还需要一系列的判断
...
function shouldYieldToHost() {
  ...
  if (timeElapsed < frameInterval) {
    ...
    // The main thread ha sonly been blocked for a really short amount of time;
  ...
  // may want to yeild control of the main thread, so the borwser can perform
  // high priority tasks. The main ones are painting and user input. If there's
  // a pending paint or a pending input, then we should yield. BUt if there's
  // neither, then we can yield less often whild remaining responsive. We'll
  // eventually yeild regardless. since there could be a pending paint that
  // wasn't accompanied by a call to 'requestPaint', or other main hread tasks
  // like network events.
